...
namespace sigmaengine { namespace protocol {
   ...
   void root_burn_operation::validate() const
   {
      validate_account_name( account );
      FC_ASSERT( memo.size() < SIGMAENGINE_MAX_MEMO_SIZE, "Memo is too large" );
      FC_ASSERT( amount.amount > 0, "amount cannot be negative" );
   ...
   void mining_operation::validate() const
   { try {
      validate_account_name( mine_account );
      FC_ASSERT( amount.amount > 0, "Cannot transfer a negative amount" );
      FC_ASSERT( fc::is_utf8( memo ), "Memo is not UTF8" );
   } FC_CAPTURE_AND_RETHROW( (*this) ) }

   void set_swap_account_operation::validate()const
   {
      try {
   ...
   void swap_operation::validate() const
   { try {
      validate_account_name( from );
      validate_account_name( to );
      FC_ASSERT( amount.amount > 0, "Cannot transfer a negative amount" );
