...
annotated_signed_transaction wallet_api::burn(string account, asset amount, string memo, bool broadcast )
{ try {
   FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
   FC_ASSERT( !is_locked() );
    burn_operation op;
   ...
   return my->sign_transaction( tx, broadcast );
} FC_CAPTURE_AND_RETHROW( (account)(amount)(memo)(broadcast) ) }

annotated_signed_transaction wallet_api::root_burn(string account, asset amount, string memo, bool broadcast )
{ try {
   FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
   FC_ASSERT( !is_locked() );
    root_burn_operation op;
...
map<uint32_t,applied_operation> wallet_api::get_account_pth_symbol_transfer_history( string account, string symbol, uint32_t from, uint32_t limit ) {
   FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
   
   auto result = my->_remote_db->get_account_pth_symbol_transfer_history(account, symbol, from,limit);
...
annotated_signed_transaction wallet_api::set_mining_account(string account, bool broadcast)
{try {
...
annotated_signed_transaction wallet_api::set_swap_account(string account, bool broadcast)
{try {

   FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
   FC_ASSERT( !is_locked() );
   set_swap_account_operation op;
...
annotated_signed_transaction wallet_api::mine(string account, asset amount, string memo, bool broadcast )
{ try {
   FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
   FC_ASSERT( !is_locked() );
    check_memo( memo, get_account( account ) );
    mining_operation op;
    op.mine_account = account;
    op.amount = amount;
    op.memo = memo;
...
annotated_signed_transaction wallet_api::swap(string from, string to, asset amount, string memo, bool broadcast )
{ try {
   FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
   FC_ASSERT( !is_locked() );
    check_memo( memo, get_account( from ) );
