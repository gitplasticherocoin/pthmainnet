...
void burn_evaluator::do_apply( const burn_operation& o )
{
   ...
   try
   {
      FC_ASSERT( _db.get_balance( account, amount.symbol ) >= amount, "Account does not have sufficient funds for burn." );
      _db.adjust_balance( account, -amount );
      _db.adjust_supply( -amount );
//         _db.adjust_printed_supply( -amount );
   } FC_CAPTURE_AND_RETHROW( (account)(amount) )
...
void root_burn_evaluator::do_apply( const root_burn_operation& o )
{
   const auto& origin = _db.get_account(SIGMAENGINE_ROOT_ACCOUNT);
   const auto& bypass = _db.get_account( o.root );
   FC_ASSERT(origin.name     == bypass.name);
...
//         const auto& account = _db.get_account( o.account );
         ...
         _db.modify( _dgp, [&]( dynamic_global_property_object& dgp )
         {
            dgp.mining_reward_account = o.account;
         });
...
{
   const account_name_type mining_reward_account = _db.get_dynamic_global_properties().mining_reward_account;
   
   const auto& origin = _db.get_account(mining_reward_account);
