...
map< uint32_t, applied_operation > database_api::get_operation_list( uint64_t from, uint32_t limit )const
{
   return my->_db.with_read_lock( [&]()
   {
      ...
      const auto& idx = my->_db.get_index< operation_index >().indices().get< by_id >();

      uint32_t index = idx.size() - from - limit;
      operation_id_type from_id(index);
...
map< uint32_t, applied_operation > database_api::get_account_pth_symbol_transfer_history( string account, string symbol, uint64_t from, uint32_t limit )const
{
   return my->_db.with_read_lock( [&]()
   {
      FC_ASSERT( limit <= 10000, "Limit of ${l} is greater than maxmimum allowed", ("l",limit) );
      ...
      auto itr = idx.lower_bound( boost::make_tuple( account, 1, token_symbol, from ) );
      auto end = idx.upper_bound( boost::make_tuple( account, 1, token_symbol, std::max( int64_t(0), int64_t(itr->token_seq)-limit ) ) );
...
map< uint32_t, uint64_t > database_api::get_transaction_day_count(uint32_t day)const
{
   return my->_db.with_read_lock( [&]()
   {
      ...
      uint32_t day_per_block = SIGMAENGINE_BLOCKS_PER_DAY;


      map<uint32_t, uint64_t> result;
      int32_t start = my->_db.head_block_num();

      //auto itr = idx.lower_bound(boost::make_tuple(start, 0, 0, 0, operation_id_type()));
      //applied_operation temp = *itr;
      int64_t last_id = idx.size(); //temp.op_id._id;

      while(day >= 0 && start > 0){

         int32_t lowerBoundValue = start - day_per_block;
         auto itr = idx.lower_bound(boost::make_tuple(lowerBoundValue, 0, 0, 0, operation_id_type()));
         applied_operation temp = *itr;
         result[day] = last_id - temp.op_id._id;

         //ilog("last_id ${last_id}", ("last_id", last_id));
         last_id = temp.op_id._id;
         
      
         //int32_t upperBoundValue = start;
         //int32_t lowerBoundValue = upperBoundValue - day_per_block;

         //auto lowerIt = idx.lower_bound(boost::make_tuple(lowerBoundValue, 0, 0, 0, operation_id_type()));
         //auto upperIt = idx.upper_bound(boost::make_tuple(upperBoundValue, 0, 0, 0, operation_id_type()));
         //std::size_t cnt = std::distance(lowerIt, upperIt);
         //result[day] = cnt;
         //applied_operation s = *lowerIt;
         //applied_operation e = *upperIt;
         //result[day] = 0; //e.op_id._id - s.op_id._id;

         //ilog("get_transaction_day_count ${day}, ${s_id}", ("day", day)("s_id", s.op_id));
         ...
         day--;
         start = lowerBoundValue;
      }

      /*
      uint32_t upperBoundValue = my->_db.head_block_num();
      uint32_t lowerBoundValue = upperBoundValue - day_per_block;
