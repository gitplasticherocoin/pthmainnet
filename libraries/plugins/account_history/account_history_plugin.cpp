   ...
   void operator()( Op&& )const
   {
         ...
         switch(_note.op.which())
         {
            case operation::tag<transfer_operation>::value:
               token_symbol = (uint64_t(SIGMAENGINE_BLOCKCHAIN_PRECISION_DIGITS) | (uint64_t('T') << 8) | (uint64_t('T') << 16) | (uint64_t('T') << 24) );
               op_tag = 1;
               break;
            ...
            case operation::tag<burn_operation>::value:
            case operation::tag<mining_operation>::value:
            case operation::tag<swap_operation>::value:
               op_tag = 1;
               break;
